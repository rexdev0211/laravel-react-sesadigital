...
class ArtisanRatingController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $query = ArtisanRating::with(['user' => function ($q)  {
    ...
    public function changeStatus(Request $request) {
        ...
        if($ids && count($ids)>0){
            ...
            ArtisanRating::whereIn('id',$ids)->update($data);
            $response = [
                'status'=>true,
                'message'=>'Record updated successfully.'
            ];
    ...
    public function destroy($id) {
        $record = ArtisanRating::findOrFail($id);
        $record->delete();
        $response = [
            'status'=>true,
    ...
    public function destroyAll(Request $request) {
        $ids = array_filter($request->ids);
        if($ids && ArtisanRating::destroy($ids)) {
            $response = [
                'status'=>true,
                'message'=>'Record deleted successfully.'
            ];
